...
Состояние в `localStorage['ui.presentationMode']`, пробел по-прежнему крутит. Тема с `hidesSidebar` (сейчас
«Тайвань») включает режим сама, когда становится активной (`App.tsx` следит за `useActiveWheelStyle`); только при
смене стиля, поэтому открытую вручную панель она больше не прячет. Переключателя языка нет:
