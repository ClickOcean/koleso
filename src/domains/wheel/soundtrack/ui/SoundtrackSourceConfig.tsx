...
import { IconPlayerPlayFilled as PlayArrowIcon } from '@tabler/icons-react';
import { IconPlayerStopFilled as StopIcon } from '@tabler/icons-react';
...
import { DEFAULT_SOUNDTRACK_CONFIG } from '@domains/wheel/soundtrack/lib/constants';
import { useAudioPlayback } from '@domains/wheel/soundtrack/lib/useAudioPlayback';
...
const SoundtrackSourceConfig: FC<SoundtrackSourceConfigProps> = ({ onClose }) => {
  ...
  // YouTube sources are created without a known duration; take it from the player once loaded
  const handleDurationChange = useCallback(
    (duration: number) => {
      if (source && Math.abs((source.duration ?? 0) - duration) > 1) {
        setValue('soundtrack.source', { ...source, duration });
      }
    },
    [setValue, source],
  );
